...
export async function seedFixture(name) {
    await cleanTestData();
    switch (name) {
...
export async function createStaffWithMessages() {
    ...
    const messages = await Promise.all(Array.from({ length: 5 }, (_, i) => db.message.create({
        data: {
            schoolId: base.school.id,
...
export async function createParentWithPayments() {
    ...
    const paymentItems = await Promise.all(Array.from({ length: 3 }, (_, i) => db.paymentItem.create({
        data: {
            schoolId: base.school.id,
