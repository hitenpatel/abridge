...
export async function sendPasswordResetEmail(data: PasswordResetEmailData) {
	try {
		const { data: emailData, error: emailError } = await getResend().emails.send({
			...
			subject: "Reset your Abridge password",
			html: `
<!DOCTYPE html>
<html>
<head>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #f5f5f5; padding: 40px 0;">
                    ...
                    <tr>
                        <td style="padding: 40px;">
                            ...
                            <table cellpadding="0" cellspacing="0" style="margin: 0 auto;">
                                <tr>
                                    <td align="center" style="background: linear-gradient(135deg, #f56e3d 0%, #ff8f6b 100%); border-radius: 6px;">
                                        <a href="${data.resetUrl}" style="display: inline-block; padding: 16px 40px; color: #ffffff; text-decoration: none; font-size: 16px; font-weight: 600;">
                                            Reset Password
                                        </a>
                                    </td>
                                </tr>
                            ...
                            <p style="margin: 30px 0 0 0; color: #999999; font-size: 14px; line-height: 1.6;">
                                ...
                                <a href="${data.resetUrl}" style="color: #f56e3d; word-break: break-all;">${data.resetUrl}</a>
                            </p>
		...
		if (emailError) {
			logger.error(
				...
				"Resend API returned error for password reset email",
			);
