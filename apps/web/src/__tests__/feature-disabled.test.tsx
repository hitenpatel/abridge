...
describe("FeatureDisabled", () => {
	it("renders feature name in heading", () => {
		...
		expect(screen.getByRole("heading", { name: /Payments is not enabled/i })).toBeInTheDocument();
	});

	it("shows disabled message text", () => {
		render(<FeatureDisabled featureName="Messaging" />);

		expect(screen.getByText(/This feature has been disabled for your school/i)).toBeInTheDocument();
	});
});
