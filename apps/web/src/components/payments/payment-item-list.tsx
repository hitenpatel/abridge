...
export function PaymentItemList({ schoolId }: PaymentItemListProps) {
	...
	if (!data || data.data.length === 0) return <div className="text-center py-4 text-gray-500">No payment items created yet.</div>;
				...
				<tbody className="bg-white divide-y divide-gray-200">
					{data.data.map((item) => (
						<tr key={item.id}>
							...
							<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">£{(item.amount / 100).toFixed(2)}</td>
							<td className="px-6 py-4 whitespace-nowrap text-sm">
								<span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${BADGE_COLORS[item.category] || "bg-gray-100 text-gray-800"}`}>
									{item.category}
								</span>
							</td>
