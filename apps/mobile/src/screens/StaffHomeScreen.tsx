...
export function StaffHomeScreen() {
	...
	return (
		<View className="flex-1 bg-background">
			{/* Header — always visible so testIDs are accessible during loading */}
			<View
				className="px-6 pb-6"
				style={{
			...
			{(__DEV__ || process.env.EXPO_PUBLIC_E2E) && (
				<View className="flex-row flex-wrap gap-2 px-6 pt-2">
					<Pressable
						testID="nav-messages"
						onPress={() => navigation.navigate("StaffMessages" as never)}
					...
					<Pressable
						...
						onPress={() => navigation.navigate("StaffAttendance" as never)}
						className="bg-neutral-surface rounded-full px-3 py-1"
					>
					...
					<Pressable
						testID="nav-payments"
						onPress={() => navigation.navigate("StaffPayments" as never)}
						className="bg-neutral-surface rounded-full px-3 py-1"
					>
